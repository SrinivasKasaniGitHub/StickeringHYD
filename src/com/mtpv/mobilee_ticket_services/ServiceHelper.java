...
public class ServiceHelper {
	...
	public static void getWheeler() {
		...
		try {
			...
			// STEP1. Opdata_Chalana is a String
			Log.i("**WHEELER_RESPONSE***", "" + Opdata_Chalana);
			// !2@2-WHEELER!3@3-WHEELER!4@4-WHEELER!L@LORRY/TRUCK!T@TRACTOR/ROAD
			// ROLLER
			if (Opdata_Chalana == null) {
				Log.i("NO_WHEELER_DATA_FOUND", "NO_WHEELER_DATA_FOUND");
			} else {
				// STEP2. now putting Opdata_Chalana in to String[] after
				// splitting at "!"
	...
	public static void getRTADetails(String vhle_num) {
		try {
			...
			Log.i("getRTADetails_response :::", "" + Opdata_Chalana);
			if (Opdata_Chalana == "0") {
				Log.i("NO_DATA_FOUND", "NO_DATA_FOUND");
			} else {
				...
				if (Dashboard.rta_details_request_from.equals("RTACLASS")) {
					...
					Drunk_Drive.rta_details_master = Opdata_Chalana.split("!");
					Log.i("**getRTADetails***", "" + Opdata_Chalana);
					...
					SpotChallan.rta_details_spot_master = Opdata_Chalana.split("!");
					Log.i("**getRTADetails SPOT***", "" + Opdata_Chalana);
	...
	public static void getpendingChallansByRegNo(String regno, String drvr_lcno, String ownr_lcnce_no, String pid,
		...
		String srickerNo = ip_val != null ? ip_val: "";
		Log.i("sticker", ""+ ip_val);
...
//		}
		...
		try {
			...
			request.addProperty("ip", "" + srickerNo); // if no pending challans then SpotChallan.sticker value is "" otherwise it sends sticker value. 
			
			request.addProperty("unitCode", "" + Dashboard.UNIT_CODE);
			System.out.println(ServiceHelper.pending_challans_details.length);
	...
	public static void getLicenceDetails(String lcnce_num) {
		...
		try {
			...
			Log.i("getLicenceDetails_request :::", "" + request);
			SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(SoapEnvelope.VER11);
			envelope.dotNet = true;
			envelope.setOutputSoapObject(request);
			...
			// Opdata_Chalana = "";
			// Opdata_Chalana = result.toString();
			Log.i("**getLiceneDetails_Response***", "" + result.toString());
			// if (Opdata_Chalana.toString().trim().equals("anyType{}"))

			try {
				Opdata_Chalana = new com.mtpv.mobilee_ticket_services.PidSecEncrypt().decrypt(result.toString());
				Log.i("**getLiceneDetailsDecryptedResponse***", "" + Opdata_Chalana);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			if (Opdata_Chalana.toString().trim().equals("0")) {
				...
				if (Dashboard.licence_details_request_from.equals("RTACLASS")) {
					...
					// Opdata_Chalana);
					Drunk_Drive.licene_details_master = new String[0];
					Drunk_Drive.licene_details_master = Opdata_Chalana.split("!");
					Log.i("**getLiceneDetails***", "" + Opdata_Chalana);

				} else if (Dashboard.licence_details_request_from.equals("SPOT")) {
					// Log.i("**getLiceneDetails***", "else--" +
					// Opdata_Chalana);
					SpotChallan.licence_details_spot_master = new String[0];
					SpotChallan.licence_details_spot_master = Opdata_Chalana.split("!");
					Log.i("**getLiceneDetails***", "" + Opdata_Chalana);
				}
			}
		} catch (SoapFault fault) {
	...
	public static void getAadharDetails(String uid, String eid) {
		...
		try {
			...
			// String Opdata_Chalana =result.toString();
	...
	public static void vehicleHistory(String vhle_hist_penchallans, String pen_challans, String regn_No, String gtwy_cd,
		...
		Log.i("****vhle_hist_penchallans***", "" + vhle_hist_penchallans != null && vhle_hist_penchallans.length() > 0
