...
public class VehicleHistoryPendingChallans extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (ServiceHelper.pending_challans_details.length > 0) {
			...
			for (int i = 0; i < (ServiceHelper.pending_challans_details.length); i++) {
				...
				int identifier = getResources().getIdentifier(getApplicationContext().getPackageName()
				...
				if(SpotChallan.cardFLG){
					...
					SpotChallan.sb_selected_penlist.add(""+ ServiceHelper.pending_challans_details[i][0].toString().trim()
					...
					SpotChallan.sb_selected_penlist.add(""+ ServiceHelper.pending_challans_details[i][1].toString().trim()+ "@"+
							 ServiceHelper.pending_challans_details[i][7].toString().trim()+ "@");
				...
				cb[i].setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (detained_items_status.get(check.getId()) == true) {
							...
							total_amount_selected_challans = total_amount_selected_challans 
									+ (Double.parseDouble(ServiceHelper.pending_challans_details[check.getId()][7].toString().trim()));
							
							Log.i("total_amount_selected_challans :::", ""+total_amount_selected_challans);
							Log.i("SpotChallan.sb_selected_penlist_positions :::", ""+SpotChallan.sb_selected_penlist_positions);
							...
							if(!SpotChallan.btn_violation.getText().toString().equals("" + getResources().getString(R.string.select_violation))){
								SpotChallan.tv_grand_total_spot.setText("Rs. " + (VehicleHistoryPendingChallans.total_amount_selected_challans+SpotChallan.grand_total));
							} else {
	...
	protected Dialog onCreateDialog(int id) {
		...
		case PENDING_CHALLAN_DETAILS:
			...
			TextView tv_detailed_pen_challan_header = (TextView) dg_details.findViewById(R.id.textView_penchallandetails_header);
			...
			TextView tv_offencedate_details = (TextView) dg_details.findViewById(R.id.tv_offencedate_pchallans_details_xml);
			TextView tv_offcencetime_details = (TextView) dg_details.findViewById(R.id.tv_offencetime_pchallans_details_xml);
			...
			TextView tv_offence_desc_details = (TextView) dg_details.findViewById(R.id.tv_offencedesc_pchallans_details_xml);
			...
			tv_vhle_no_details.setText(""+ (ServiceHelper.pending_challans_details[newpos][0].equals("0") ? " ": ServiceHelper.pending_challans_details[newpos][0]));
			tv_etckt_no_details.setText(""+ (ServiceHelper.pending_challans_details[newpos][1].equals("0") ? " ": ServiceHelper.pending_challans_details[newpos][1]));
			tv_offencedate_details.setText(""+ (ServiceHelper.pending_challans_details[newpos][2].equals("0") ? " ": ServiceHelper.pending_challans_details[newpos][2]));
			tv_offcencetime_details.setText(""+ (ServiceHelper.pending_challans_details[newpos][3].equals("0") ? " ": ServiceHelper.pending_challans_details[newpos][3]));
			tv_pointname_details.setText(""+ (ServiceHelper.pending_challans_details[newpos][4].equals("0") ? " ": ServiceHelper.pending_challans_details[newpos][4]));
			tv_psname_details.setText(""+ (ServiceHelper.pending_challans_details[newpos][5].equals("0") ? " ": ServiceHelper.pending_challans_details[newpos][5]));
			tv_offence_desc_details.setText(""+ (ServiceHelper.pending_challans_details[newpos][6].equals("0") ? " ": ServiceHelper.pending_challans_details[newpos][6]));
			tv_amnt_details.setText("Rs : "+ (ServiceHelper.pending_challans_details[newpos][7].equals("0") ? " ": ServiceHelper.pending_challans_details[newpos][7]));
			...
			if (!ServiceHelper.pending_challans_details[newpos][8].toString().trim().equals("0")) {
				...
				imageLoader.displayImage(ServiceHelper.pending_challans_details[newpos][8].toString().trim(), img_display);
			}
	...
	public void onBackPressed() {
		...
		if (sb_selected_penlist_positions.size() > 0) {
			for (int i = 0; i < sb_selected_penlist_positions.size(); i++) {
				sb_selected_penlist_send.append(""+ sb_selected_penlist.get(Integer.parseInt(sb_selected_penlist_positions.get(i))));
			}
		}
...
/*		for (int i = 0; i < split1.length; i++) {
			...
			for (int j = 0; j < challanSelected.length; j++) {
				if (j==1) {
					ticket = challanSelected[1];
				}else if (j==9) {
					amount = challanSelected[8];
				}
			}
		...
		for (int i = 0; i < split1.length; i++) {
			...
			for (int j = 0, k = 1; j < challanSelected.length; j++, k++) {
				count++;
				/*if (j == 1) {
					ticket = challanSelected[0];
					amount = challanSelected[1];
					Log.i("ticket", ""+ticket);
					Log.i("amount", ""+amount);
				} else if (j == 9) {
					amount = challanSelected[8];
		...
		if (completeSplit != null && ticket != null && amount != null) {
			AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(VehicleHistoryPendingChallans.this, AlertDialog.THEME_HOLO_LIGHT);
